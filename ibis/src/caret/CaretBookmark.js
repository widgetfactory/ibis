 ...
 * @private
 * @static
 * @class ibis.caret.CaretBookmark
 * @example
 ...
 */
(function (ibis) {
  var NodeType = ibis.dom.NodeType, DOMUtils = ibis.DOM;
  var Fun = ibis.util.Fun, Arr = ibis.util.Arr;
  ...
  function findTextPosition(container, offset) {
    ...
    return new ibis.caret.CaretPosition(container, offset);
  }
  ...
  function resolve(rootNode, path) {
    ...
    if (!isText(container)) {
      ...
      return new ibis.caret.CaretPosition(container.parentNode, offset);
    }

    return findTextPosition(container, parseInt(offset, 10));
  }

  ibis.caret.CaretBookmark = {
    /**
     * Create a xpath bookmark location for the specified caret position.
     *
     * @method create
     * @param {Node} rootNode Root node to create bookmark within.
     * @param {ibis.caret.CaretPosition} caretPosition Caret position within the root node.
     ...
     * @return {ibis.caret.CaretPosition} Caret position resolved from xpath like bookmark.
     */
    resolve: resolve
  };
