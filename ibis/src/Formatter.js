...
(function (ibis) {
   ...
   *
   * @class ibis.Formatter
   * @example
   *  ibis.activeEditor.formatter.register('mycustomformat', {
   *    inline : 'span',
   *    styles : {color : '#ff0000'}
   *  });
   *
   *  ibis.activeEditor.formatter.apply('mycustomformat');
   */
   ...
   * @param {ibis.Editor} ed Editor instance to construct the formatter engine to.
   */
  ibis.Formatter = function (ed) {
    var formats = {},
      each = ibis.each,
      dom = ed.dom,
      selection = ed.selection,
      TreeWalker = ibis.dom.TreeWalker,
      rangeUtils = new ibis.dom.RangeUtils(dom),
        ...
        rangeUtils.walk(rng, function (nodes) {
          ...
          function process(node) {
            ...
            if (contentEditable && !hasContentEditableState && isValidChild(wrapName, nodeName) && isValidChild(parentName, wrapName) &&
              ...
              each(ibis.grep(node.childNodes), process);
        ...
        if (node && node.parentNode) {
          ...
          if (!isCollapsed || !format.inline || dom.select('td.mceSelected,th.mceSelected,div.mceSelected').length) {
            ...
            if (format.styles && (format.styles.color || format.styles.textDecoration)) {
              ibis.walk(curSelNode, processUnderlineAndColor, 'childNodes');
    ...
    function remove(name, vars, node, similar) {
      ...
      function process(node) {
        ...
        children = ibis.grep(node.childNodes);
    ...
    function formatChanged(formats, callback, similar) {
      ...
      if (!formatChangeData) {
        ...
        ed.onNodeChange.addToTop(function (ed, cm, node) {
          ...
          parents = ibis.grep(parents, function (node) {
        ...
        if (!forcedRootBlock) {
          ...
          if (parentNode == dom.getRoot()) {
            if (!format.list_block || !isEq(node, format.list_block)) {
              each(ibis.grep(node.childNodes), function (node) {
    ...
    function mergeSiblings(prev, next) {
      ...
      if (prev && next) {
        ...
        if (compareElements(prev, next)) {
          ...
          each(ibis.grep(next.childNodes), function (node) {
    ...
    function performCaretAction(type, name, vars, similar) {
      ...
      function unmarkBogusCaretParents() {
        ...
        if (caretContainer && !dom.isEmpty(caretContainer)) {
          ibis.walk(caretContainer, function (node) {
      ...
      if (!ed._hasCaretEvents) {
        ...
        ibis.each('onMouseUp onKeyUp'.split(' '), function (name) {
