...
(function (ibis) {
  ...
  var DOM = ibis.DOM,
    ...
    extend = ibis.extend,
    each = ibis.each,
    isGecko = ibis.isGecko,
    isIE = ibis.isIE,
    is = ibis.is,
    ThemeManager = ibis.ThemeManager,
    PluginManager = ibis.PluginManager,
    EditorFocus = ibis.EditorFocus,
    explode = ibis.explode,
    Zwsp = ibis.text.Zwsp,
    TrimBody = ibis.dom.TrimBody;
   ...
   * ibis.activeEditor.dom.addClass(ibis.activeEditor.dom.select('p'), 'someclass');
   *
   * // Gets the current editors selection as text
   * ibis.activeEditor.selection.getContent({format : 'text'});
   *
   * // Creates a new editor instance
   * var ed = new ibis.Editor('textareaid', {
   *     some_setting : 1
   * });
   *
   ...
   * ed.render();
   */
  ibis.Editor = function (id, settings) {
    var self = this,
      TRUE = true;
     ...
     * ibis.activeEditor.windowManager.alert("You are using the " + ibis.activeEditor.settings.theme + " theme");
     */
    self.settings = settings = extend({
      ...
      popup_css: '',
      plugins: '',
      document_base_url: ibis.documentBaseURL,
     ...
     * ibis.activeEditor.plugins.someplugin.someMethod();
     */
    self.plugins = {};
     ...
     * ibis.activeEditor.documentBaseURI.toRelative('/somedir/somefile.htm');
     *
     * // Get absolute URL from the location of document_base_ur\
     * ibis.activeEditor.documentBaseURI.toAbsolute('somefile.htm');
     */
    self.documentBaseURI = new ibis.util.URI(settings.document_base_url || ibis.documentBaseURL, {
      base_uri: ibis.baseURI
    });
     ...
     * ibis.activeEditor.baseURI.toRelative('/somedir/somefile.htm');
     *
     * // Get absolute URL from the location of the API
     * ibis.activeEditor.baseURI.toAbsolute('somefile.htm');
     */
    self.baseURI = ibis.baseURI;
  ...
  ibis.Editor.prototype = {
    ...
    render: function () {
       ...
       * @type ibis.WindowManager
       * @example
       * // Shows an alert message
       * ibis.activeEditor.windowManager.alert('Hello world!');
       *
       * // Opens a new dialog with the file.htm file and the size 320x240
       * // It also adds a custom parameter this can be retrieved by using ibisPopup.getWindowArg inside the dialog.
       * ibis.activeEditor.windowManager.open({
       *    url : 'file.htm',
       *    width : 320,
       *    height : 240
       * }, {
       ...
       * });
       */
      if (ibis.WindowManager) {
        self.windowManager = new ibis.WindowManager(self);
      }
      ...
      if (s.add_unload_trigger) {
        self._beforeUnload = ibis.onBeforeUnload.add(function () {
      ...
      ibis.addUnload(self.destroy, self);
      ...
      function loadScripts() {
        if (s.theme && typeof s.theme != "function" && s.theme.charAt(0) != '-' && !ThemeManager.urls[s.theme]) {
          ThemeManager.load(s.theme, 'themes/' + s.theme + '/theme' + ibis.suffix + '.js');
        }
    ...
    init: function () {
       ...
       * ibis.activeEditor.theme.someMethod();
       */
      if (s.theme) {
        if (typeof s.theme != "function") {
          ...
          if (self.theme.init) {
            self.theme.init(self, ThemeManager.urls[s.theme] || ibis.documentBaseURL.replace(/\/$/, ''));
          }
        } else {
          self.theme = s.theme;
        }
      ...
      function initPlugin(p) {                
        ...
        if (c && ibis.inArray(initializedPlugins, p) === -1) {
       ...
       * @type ibis.ControlManager
       * @example
       * // Disables the bold button
       * ibis.activeEditor.controlManager.setDisabled('bold', true);
       */
      self.controlManager = new ibis.ControlManager(self);
      ...
      if (document.domain && location.hostname != document.domain) {
        ibis.relaxedDomain = document.domain;
      }
      ...
      if (s.document_base_url != ibis.documentBaseURL) {
      ...
      /*for (i = 0; i < self.contentCSS.length; i++) {
        var cssUrl = ibis._addVer(self.contentCSS[i]);
      ...
      if (ibis.relaxedDomain) {
        // We need to write the contents here in IE since multiple writes messes up refresh button and back button
        url = 'javascript:(function(){document.open();document.domain="' + document.domain + '";var ed = window.parent.ibis.get("' + self.id + '");document.write(ed.iframeHTML);document.close();ed.initContentBody();})()';
      }
    ...
    initContentBody: function () {
      ...
      // Setup iframe body
      if ((!isIE || !ibis.relaxedDomain) && !settings.content_editable) {
        doc.open();
        doc.write(self.iframeHTML);
        doc.close();

        if (ibis.relaxedDomain) {
          doc.domain = ibis.relaxedDomain;
        }
      }
       ...
       *
       * @property schema
       * @type ibis.html.Schema
       */
      self.schema = new ibis.html.Schema(settings);

      /**
       * DOM instance for the editor.
       *
       * @property dom
       * @type ibis.dom.DOMUtils
       * @example
       * // Adds a class to all paragraphs within the editor
       * ibis.activeEditor.dom.addClass(ibis.activeEditor.dom.select('p'), 'someclass');
       */
      self.dom = new ibis.dom.DOMUtils(doc, {
        keep_values: true,
        url_converter: self.convertURL,
        url_converter_scope: self,
      ...
      self.parser.addNodeFilter('p,h1,h2,h3,h4,h5,h6,div', function (nodes) {
        ...
        while (i--) {
          ...
          if (node.isEmpty(nonEmptyElements)) {
            node.empty().append(new ibis.html.Node('br', 1)).shortEnded = true;
          }
        }
      });
       ...
       *
       * @property serializer
       * @type ibis.dom.Serializer
       * @example
       * // Serializes the first paragraph in the editor into a string
       * ibis.activeEditor.serializer.serialize(ibis.activeEditor.dom.select('p')[0]);
       */
      self.serializer = new ibis.dom.Serializer(settings, self.dom, self.schema);

      /**
       * Selection instance for the editor.
       *
       * @property selection
       * @type ibis.dom.Selection
       * @example
       * // Sets some contents to the current selection in the editor
       * ibis.activeEditor.selection.setContent('Some contents');
       *
       * // Gets the current selection
       * alert(ibis.activeEditor.selection.getContent());
       *
       * // Selects the first paragraph found
       * ibis.activeEditor.selection.select(ibis.activeEditor.dom.select('p')[0]);
       */
      self.selection = new ibis.dom.Selection(self.dom, self.getWin(), self.serializer, self);
       ...
       * @type ibis.Formatter
       */
      self.formatter = new ibis.Formatter(self);

      /**
      * Textpattern instance.
      *
      * @property textpattern
      * @type ibis.Textpattern
      */
      self.textpattern = new ibis.TextPattern(self);

      /**
       * Undo manager instance, responsible for handling undo levels.
       /
       * @property undoManager
       * @type ibis.UndoManager
       * @example
       * // Undoes the last modification to the editor
       * ibis.activeEditor.undoManager.undo();
       */
      self.undoManager = new ibis.UndoManager(self);
      ...
      // internal with no external interface
      self._selectionOverrides = new ibis.SelectionOverrides(self);
      // internal with no external interface
      self._clipBoard = new ibis.Clipboard(self);
      ...
      self.quirks = ibis.util.Quirks(self);
        ...
        if (settings.auto_focus) {
          setTimeout(function () {
            ...
            if (settings.auto_focus === true) {
              ...
              focusEditor = ibis.get(settings.auto_focus);
            }
      ...
      var styleLoader = new ibis.dom.StyleSheetLoader(self.getDoc());
    ...
    /**
     * Focuses/activates the editor. This will set this editor as the activeEditor in the ibis collection
    ...
    getLang: function (n, dv) {
      return ibis.i18n[(this.settings.language || 'en') + '.' + n] || (is(dv) ? dv : '{#' + n + '}');
    },
     ...
     * var someval = ibis.activeEditor.getParam('myvalue');
     *
     * // Returns a specific config value from a specific editor instance by id
     * var someval2 = ibis.get('my_editor').getParam('myvalue');
     */
    ...
    addShortcut: function (pattern, desc, cmdFunc, scope) {
      ...
      var modifierNames = ibis.makeMap('alt,ctrl,shift,meta,access');
      ...
      function createShortcut(pattern, desc, cmdFunc, scope) {
        ...
        shortcuts = ibis.map(explode(pattern, '>'), parseShortcut);
      ...
      } else if (ibis.isArray(cmd)) {
        cmdFunc = function () {
          self.execCommand(cmd[0], cmd[1], cmd[2]);
        };
      }

      each(explode(ibis.trim(pattern.toLowerCase())), function (pattern) {
     ...
     * ibis.activeEditor.setProgressState(true);
     *
     * // Hide progress for the active editor
     * ibis.activeEditor.setProgressState(false);
     *
     * // Show progress after 3 seconds
     * ibis.activeEditor.setProgressState(true, 3000);
     */
     ...
     * ibis.activeEditor.setContent('<span>some</span> html');
     *
     * // Sets the raw contents of the activeEditor editor
     * ibis.activeEditor.setContent('<span>some</span> html', {format : 'raw'});
     *
     * // Sets the content of a specific editor (my_editor in this example)
     * ibis.get('my_editor').setContent(data);
     *
     * // Sets the bbcode contents of the activeEditor editor if the bbcode plugin was added
     * ibis.activeEditor.setContent('[b]some[/b] html', {format : 'bbcode'});
     */
    setContent: function (content, args) {
      ...
      if (content.length === 0 || /^\s+$/.test(content)) {
        ...
        if (args.format !== 'raw') {
          content = new ibis.html.Serializer({}, self.schema).serialize(
            self.parser.parse(content)
          );
        }

        // Set the new cleaned contents to the editor
        args.content = ibis.trim(content);
     ...
     * console.debug(ibis.activeEditor.getContent());
     *
     * // Get the raw contents of the currently active editor
     * ibis.activeEditor.getContent({format : 'raw'});
     *
     * // Get content of a specific editor:
     * ibis.get('content id').getContent()
     */
    getContent: function (args) {
      ...
      if (args.format == 'raw') {
        content = ibis.trim(Zwsp.trim(TrimBody.trim(body, self.serializer.getTempAttrs()).innerHTML));
    ...
    /**
     * Removes the editor from the dom and ibis collection.
     *
     * @method remove
     */
    remove: function () {
    ...
    destroy: function (s) {
      ...
      if (!s) {
        ibis.removeUnload(self.destroy);
        ibis.onBeforeUnload.remove(self._beforeUnload);
