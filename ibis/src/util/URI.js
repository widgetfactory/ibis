...
(function (ibis) {
   ...
   * @class ibis.util.URI
   */
  ibis.util.URI = function (url, settings) {
    var self = this,
      baseUri, base_url;
    ...
    if (!/^[\w\-]*:?\/\//.test(url)) {
      base_url = settings.base_uri ? settings.base_uri.path : new ibis.util.URI(location.href).directory;
  ...
  ibis.util.URI.prototype = {
     ...
     * var url = new ibis.util.URI('http://www.site.com/dir/').toRelative('http://www.site.com/dir/somedir/somefile.htm');
     */
     ...
     * var url = new ibis.util.URI('http://www.site.com/dir/').toAbsolute('somedir/somefile.htm');
     */
     ...
     *
     * @method isSameOrigin
     * @param {ibis.util.URI} uri Uri instance to compare.
  ...
  var blockSvgDataUris = function (allowSvgDataUrls, tagName) {
    if (isNonNullable(allowSvgDataUrls)) {
      ...
      return isNonNullable(tagName) ? ibis.inArray(safeSvgDataUrlElements, tagName) == -1 : true;
    }
  };
  ...
  ibis.util.URI.parseDataUri = function (uri) {
    var type, matches;
  ...
  ibis.util.URI.getDocumentBaseUrl = function (loc) {
    var baseUrl;
   ...
   */
  ibis.util.URI.isDomSafe = function (uri, context, options) {
    if (options.allow_script_urls) {
      return true;
    } else {
      // eslint-disable-next-line no-control-regex
      let decodedUri = ibis.html.Entities.decode(uri).replace(/[\s\u0000-\u001F]+/g, '');
