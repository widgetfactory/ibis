 ...
 * ibis.activeEditor.on('ShowCaret BeforeObjectSelected', function(e) {
 *     e.preventDefault();
 * });
 *
 * @private
 * @class ibis.SelectionOverrides
 */
(function (ibis) {

  var CaretWalker = ibis.caret.CaretWalker, CaretPosition = ibis.caret.CaretPosition, CaretContainer = ibis.caret.CaretContainer, CaretUtils = ibis.caret.CaretUtils,
    CaretContainerRemove = ibis.caret.CaretContainerRemove, FakeCaret = ibis.caret.FakeCaret, LineWalker = ibis.caret.LineWalker, LineUtils = ibis.caret.LineUtils,
    ClientRect = ibis.geom.ClientRect;

  var NodeType = ibis.dom.NodeType, RangeUtils = ibis.dom.RangeUtils;
  var VK = ibis.VK, Fun = ibis.util.Fun, Arr = ibis.util.Arr;

  var Dispatcher = ibis.util.Dispatcher, DragDropOverrides = ibis.DragDropOverrides;
  ...
  ibis.SelectionOverrides = function (editor) {
      ...
      editor.onCopy.add(function (editor, e) {
        ...
        if (!e.isDefaultPrevented() && e.clipboardData) {
          ...
          if (realSelectionElement) {
            ...
            clipboardData.setData('x-ibis/html', html);
