...
(function (ibis) {
  ...
  var each = ibis.each,
    ...
    BookmarkManager = ibis.dom.BookmarkManager,
    ControlSelection = ibis.dom.ControlSelection,
    RangeUtils = ibis.dom.RangeUtils,
    CaretPosition = ibis.caret.CaretPosition,
    Zwsp = ibis.text.Zwsp,
    ScrollIntoView = ibis.dom.ScrollIntoView,
    Env = ibis.util.Env;
   ...
   *
   * @class ibis.dom.Selection
   * @example
   * // Getting the currently selected node for the active editor
   * alert(ibis.activeEditor.selection.getNode().nodeName);
   */
  /**
     ...
     * @constructor
     * @method Selection
     * @param {ibis.dom.DOMUtils} dom DOMUtils object reference.
     * @param {Window} win Window to bind the selection object to.
     * @param {ibis.dom.Serializer} serializer DOM serialization class to use for getContent.
     */
  ibis.dom.Selection = function (dom, win, serializer, editor) {
    var self = this;
    ...
    each([
       ...
       * @param {ibis.dom.Selection} selection Selection object that fired the event.
      ...
      self[e] = new ibis.util.Dispatcher(self);
    });
   ...
   */
    ...
    getContextualFragment: function (rng, frag) {
      ...
      var elms = ibis.grep(parents, function (elm) {
     ...
     * alert(ibis.activeEditor.selection.getContent({format: 'text'}));
     */
     ...
     * ibis.activeEditor.selection.setContent('<strong>Some contents</strong>');
     */
     ...
     * var bm = ibis.activeEditor.selection.getBookmark();
     *
     * ibis.activeEditor.setContent(ibis.activeEditor.getContent() + 'Some new content');
     *
     * // Restore the selection bookmark
     * ibis.activeEditor.selection.moveToBookmark(bm);
     */
     ...
     * ibis.activeEditor.selection.setNode(ibis.activeEditor.dom.create('img', {src: 'some.gif', title: 'some title'}));
     */
    ...
    getNode: function () {
      ...
      if (rng.setStart) {
        ...
        if (!rng.collapsed) {
          ...
          //if (ibis.isWebKit && sel.anchorNode && sel.anchorNode.nodeType == 1)
    ...
    _moveEndPoint: function (rng, node, start) {
      ...
      do {
        // Text node
        if (node.nodeType == 3 && ibis.trim(node.nodeValue).length !== 0) {
          if (start) {
            rng.setStart(node, 0);
          } else {
