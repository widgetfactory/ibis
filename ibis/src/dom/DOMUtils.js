...
(function (ibis) {
  ...
  var each = ibis.each,
    is = ibis.is,
    isWebKit = ibis.isWebKit,
    isIE = ibis.isIE,
    Entities = ibis.html.Entities,
   ...
   *
   * @class ibis.dom.DOMUtils
   * @example
   * // Add a class to an element by id in the page
   * ibis.DOM.addClass('someid', 'someclass');
   *
   * // Add a class to an element by id inside the editor
   * ibis.activeEditor.dom.addClass('someid', 'someclass');
   */
  /**
     ...
     */
  ibis.dom.DOMUtils = function (d, s) {
    var self = this,
      blockElementsMap;
    ...
    self.counter = 0;
    self.stdMode = !ibis.isIE || d.documentMode >= 8;
    self.boxModel = !ibis.isIE || d.compatMode == "CSS1Compat" || self.stdMode;
    self.hasOuterHTML = "outerHTML" in d.createElement("a");

    self.settings = s = ibis.extend({
      keep_values: false,
      hex_colors: 1
    }, s);

    self.schema = s.schema;
    self.styles = new ibis.html.Styles({
      url_converter: s.url_converter,
      url_converter_scope: s.url_converter_scope
    }, s.schema);

    self.events = s.ownEvents ? new ibis.dom.EventUtils(s.proxy) : ibis.dom.Event;
    ibis.addUnload(self.destroy, this);
  ...
  ibis.dom.DOMUtils.prototype = {
    ...
    clone: function (node, deep) {
      ...
      if (!isIE || ibis.isIE11 || node.nodeType !== 1 || deep) {
   ...
   */
    ...
    /**
     * Select elements by a CSS selector inside an optional scope.
     *
     * @param {String} selector
     * @param {Object|Element|Document} scope
    ...
    select: function (selector, scope) {
      ...
      const root = this.get(scope) || this.get(this.settings.root_element) || this.doc;
      const ctx = (root && (root.nodeType === 1 || root.nodeType === 9 || root.nodeType === 11)) ?
        root : this.doc;
     ...
     * ibis.activeEditor.dom.add(ibis.activeEditor.getBody(), 'p', {title : 'my title'}, 'Some content');
     */
     ...
     * var el = ibis.activeEditor.dom.create('div', {id : 'test', 'class' : 'myclass'}, 'some content');
     * ibis.activeEditor.selection.setNode(el);
     */
     ...
     * ibis.activeEditor.selection.setContent(ibis.activeEditor.dom.createHTML('a', {href : 'test.html'}, 'some line'));
     */
    createHTML: function (n, a, h) {
      ...
      // A call to ibis.is doesn't work for some odd reason on IE9 possible bug inside their JS runtime
     ...
     * ibis.activeEditor.dom.remove(ibis.activeEditor.dom.select('p'));
     *
     * // Removes a element by id in the document
     * ibis.DOM.remove('mydiv');
     */
      ...
      return this.run(node, function (node) {
        ...
        if (keep_children) {
          while ((child = node.firstChild)) {
            ...
            if (!ibis.isIE || child.nodeType !== 3 || child.nodeValue) {
     ...
     * ibis.activeEditor.dom.setStyle(ibis.activeEditor.dom.select('p'), 'background-color', 'red');
     *
     * // Sets a style value to an element by id in the current document
     * ibis.DOM.setStyle('mydiv', 'background-color', 'red');
     */
    setStyle: function (n, na, v) {
      ...
      return self.run(n, function (e) {
        ...
        if (self.pixelStyles.test(na) && (ibis.is(v, 'number') || /^[\-0-9\.]+$/.test(v))) {
          v += 'px';
        }

        if (na == 'float') {
          na = ibis.isIE && ibis.isIE < 12 ? 'styleFloat' : 'cssFloat';
        }
     ...
     * ibis.activeEditor.dom.setStyles(ibis.activeEditor.dom.select('p'), {'background-color' : 'red', 'color' : 'green'});
     *
     * // Sets styles to an element by id in the current document
     * ibis.DOM.setStyles('mydiv', {'background-color' : 'red', 'color' : 'green'});
     */
     ...
     * ibis.activeEditor.dom.setAttrib(ibis.activeEditor.dom.select('p'), 'class', 'myclass');
     *
     * // Sets an attribute to a specific element in the current page
     * ibis.dom.setAttrib('mydiv', 'class', 'myclass');
     */
    setAttrib: function (e, n, v) {
      ...
      return this.run(e, function (e) {
        ...
        if (ibis.activeEditor && originalValue != v) {
          var ed = ibis.activeEditor;
     ...
     * ibis.activeEditor.dom.setAttribs(ibis.activeEditor.dom.select('p'), {'class' : 'myclass', title : 'some title'});
     *
     * // Sets some attributes to a specific element in the current page
     * ibis.DOM.setAttribs('mydiv', {'class' : 'myclass', title : 'some title'});
     */
     ...
     * ibis.DOM.loadCSS('somepath/some.css');
     *
     * // Loads a CSS file into the currently active editor instance
     * ibis.activeEditor.dom.loadCSS('somepath/some.css');
     *
     * // Loads a CSS file into an editor instance by id
     * ibis.get('someid').dom.loadCSS('somepath/some.css');
     *
     * // Loads multiple CSS files into the current document
     * ibis.DOM.loadCSS('somepath/some.css,somepath/someother.css');
     */
    loadCSS: function (u) {
      var self = this,
        d = self.doc,
     ...
     * ibis.activeEditor.dom.addClass(ibis.activeEditor.dom.select('p'), 'myclass');
     *
     * // Adds a class to a specific element in the current page
     * ibis.DOM.addClass('mydiv', 'myclass');
     */
     ...
     * ibis.activeEditor.dom.removeClass(ibis.activeEditor.dom.select('p'), 'myclass');
     *
     * // Removes a class from a specific element in the current page
     * ibis.DOM.removeClass('mydiv', 'myclass');
     */
     ...
     * ibis.activeEditor.dom.setHTML(ibis.activeEditor.dom.select('p'), 'some inner html');
     *
     * // Sets the inner HTML of a element by id in the document
     * ibis.DOM.setHTML('mydiv', 'some inner html');
     */
      ...
      return self.run(element, function (element) {
        if (isIE) {
          ...
          try {
            ...
            each(ibis.grep(newElement.childNodes), function (node, i) {
              // Skip br element
     ...
     * @example
     * ibis.DOM.getOuterHTML(editorElement);
     * ibis.activeEditor.getOuterHTML(ibis.activeEditor.getBody());
     */
     ...
     * ibis.activeEditor.dom.setOuterHTML(ibis.activeEditor.dom.select('p'), '<div>some html</div>');
     *
     * // Sets the outer HTML of a element by id in the document
     * ibis.DOM.setOuterHTML('mydiv', '<div>some html</div>');
     */
    ...
    replace: function (n, o, k) {
      ...
      return self.run(o, function (o) {
        if (k) {
          each(ibis.grep(o.childNodes), function (c) {
            n.appendChild(c);
          });
        }
     ...
     *
     * @example
     * ibis.DOM.isEmpty(node, {img : true});
     * @method isEmpty
    ...
    isEmpty: function (node, elements) {
      ...
      if (node) {
        walker = new ibis.dom.TreeWalker(node, node.parentNode);
      ...
      function trim(node) {
        ...
        if (type != 9) {
          ...
          if (type == 3 && node.nodeValue.length > 0) {
            ...
            var trimmedLength = ibis.trim(node.nodeValue).length;
   ...
   * ibis.DOM.addClass('someid', 'someclass');
   */
  ibis.DOM = new ibis.dom.DOMUtils(document, {
    process_html: 0
  });
})(ibis);