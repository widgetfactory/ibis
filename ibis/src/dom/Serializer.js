...
(function (ibis) {
   ...
   * @param {ibis.dom.DOMUtils} dom DOMUtils instance reference.
   * @param {ibis.html.Schema} schema Optional schema reference.
   */
  ibis.dom.Serializer = function (settings, dom, schema) {
    var self = this,
    ...
    dom = dom || ibis.DOM;
    schema = schema || new ibis.html.Schema(settings);
     ...
     *
     * @event onPreProcess
     * @param {ibis.dom.Serializer} sender object/Serializer instance that is serializing an element.
     ...
     * });
     */
    onPreProcess = new ibis.util.Dispatcher(self);
     ...
     * });
     */
    onPostProcess = new ibis.util.Dispatcher(self);

    htmlParser = new ibis.html.DomParser(settings, schema);
    ...
    htmlParser.addNodeFilter('noscript', function (nodes) {
      ...
      while (i--) {
        ...
        if (node) {
          node.value = ibis.html.Entities.decode(node.value);
        }
      }
    });
    ...
    return {
      ...
      serialize: function (node, args) {
        ...
        // Parse HTML
        rootNode = htmlParser.parse(ibis.trim(args.getInner ? clonedNode.innerHTML : dom.getOuterHTML(clonedNode)), args);
        trimTrailingBr(rootNode);

        // Serialize HTML
        htmlSerializer = new ibis.html.Serializer(settings, schema);
      ...
      addTempAttr: function (name) {
        if (ibis.inArray(tempAttrs, name) === -1) {
